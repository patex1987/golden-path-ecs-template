  ...
  return (
    <section className="panel diagnostics-panel" aria-labelledby="diagnostics-title">
      <div className="panel-header">
        ...
        <button className="icon-button" type="button" onClick={onNewWorkflow} aria-label="Start new traced workflow">
          <RotateCcw aria-hidden="true" size={18} />
        </button>
      </div>

      <div className="trace-grid">
        <TraceValue label="Correlation" value={workflow.correlationId} copyValue={workflow.correlationId} />
        <TraceValue label="Trace" value={workflow.traceId} copyValue={workflow.traceId} />
        <TraceValue
          label="Request"
        ...
        <TraceValue
          label="Reservation"
          value={reservationRequest?.id === undefined ? 'No request yet' : formatShortId(reservationRequest.id)}
      ...
      <div className="latest-operation" aria-live="polite">
        ...
        <div>
          <span>{latestExchange?.operationName ?? 'Waiting for GraphQL traffic'}</span>
          <small>
      ...
      <div className="exchange-log" aria-label="Recent GraphQL exchanges">
        {exchanges.length === 0 ? (
          ...
          exchanges.map((exchange) => (
            <div className="exchange-row" key={`${exchange.operationName}-${exchange.requestId}`}>
