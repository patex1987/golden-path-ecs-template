...
export function CatalogPanel({ catalog, isLoading, selectedMovieId, onMovieSelect, onReload }: CatalogPanelProps) {
  ...
  return (
    <section className="panel catalog-panel" aria-labelledby="catalog-title">
      <div className="panel-header">
        ...
        <button className="icon-button" type="button" onClick={onReload} aria-label="Reload catalog">
          <RefreshCw aria-hidden="true" size={18} className={isLoading ? 'spin' : undefined} />
        </button>
      </div>
      ...
      {catalog !== undefined ? (
        <div className="operator-strip" aria-label="Authenticated operator">
          ...
          <span>{catalog.me.movieProviderCode ?? formatShortId(catalog.me.movieProviderId)}</span>
        </div>
      ) : null}
...
function MovieButton({ movie, posterIndex, isSelected, onSelect }: MovieButtonProps) {
  return (
    <button
    ...
    >
      <span className={`poster-art poster-art--${(posterIndex % 4) + 1}`} aria-hidden="true">
        <span />
      </span>
...
function CatalogSkeleton() {
  return (
    <div className="skeleton-stack" aria-label="Loading catalog" aria-busy="true">
