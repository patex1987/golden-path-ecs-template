...
export function createAppComposition(
  ...
  const profile =
    overrides.compositionProfile ?? runtimeConfig.COMPOSITION_PROFILE;
  ...
  const selectedAuthMode =
    overrides.authMode ??
    (useProfileDefaults ? profileModes.authMode : runtimeConfig.AUTH_MODE);
  const selectedPersistenceMode =
    overrides.persistenceMode ??
    (useProfileDefaults
      ? profileModes.persistenceMode
      : runtimeConfig.PERSISTENCE_MODE);
  const selectedReservationWorkerMode =
    overrides.reservationWorkerMode ?? runtimeConfig.RESERVATION_WORKER_MODE;
