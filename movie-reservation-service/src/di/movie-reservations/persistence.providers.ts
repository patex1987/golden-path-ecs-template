...
import { createKnexConfig, createPostgresConnectionSettings } from '../../infrastructure/database/knex-config';
...
export function createPersistenceProviders(persistenceMode: PersistenceMode): Provider[] {
...
export function createPersistenceExports(persistenceMode: PersistenceMode): NonNullable<DynamicModule['exports']> {
  return persistenceMode === 'in-memory' ? [IN_MEMORY_MOVIE_RESERVATION_STORE] : [POSTGRES_KNEX];
}
  ...
  return [
    {
      ...
      useFactory: (): Knex => knexFactory(createKnexConfig(createPostgresConnectionSettings(config))),
    },
    PostgresKnexLifecycleService,
    {
      provide: MOVIE_RESERVATION_REPOSITORY,
      useFactory: (database: Knex): MovieReservationRepository => new PostgresMovieReservationRepository(database),
  ...
  return [
    {
      ...
      useFactory: (): InMemoryMovieReservationStore => InMemoryMovieReservationStore.withSeedData(),
    },
    {
      provide: MOVIE_RESERVATION_REPOSITORY,
      useFactory: (store: InMemoryMovieReservationStore): MovieReservationRepository =>
        new InMemoryMovieReservationRepository(store),
      inject: [IN_MEMORY_MOVIE_RESERVATION_STORE],
    },
    {
      provide: RESERVATION_REQUEST_WORK_REPOSITORY,
      useFactory: (store: InMemoryMovieReservationStore): ReservationRequestWorkRepository =>
